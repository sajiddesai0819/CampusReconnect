...
// Query to initialize the schema if tables don't exist
const initDbQuery = `
...
CREATE INDEX IF NOT EXISTS idx_items_search ON items (title, description);
        ...
        if (adminResult.rows.length === 0) {
            ...
            await client.query(
                `INSERT INTO users (username, email, password_hash, usn, role) 
                 VALUES ($1, $2, $3, $4, $5)`,
                ['admin', adminEmail, hashedPassword, 'ADMIN', 'admin']
            );
            console.log('✅ Default admin account created successfully (admin@campus.reconnect / adminpass).');
        }
    } catch (dbErr) {
        console.error('Error during database verification/initialization:', dbErr);
    } finally {
        release();
    }
});
